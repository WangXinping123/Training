	...
	public JSONObject addAfterLogin(HttpServletResponse response,@RequestBody ReceivingAddress address) {
		...
		if(cookie==null) {
			...
			if (address.getreceiverName() != null && address.getCellPhone() != null
				...
				if ((address.getreceiverName() instanceof String) && (address.getCellPhone() instanceof String)
						&& (address.getCellPhone().toString().length() == 11)
						&& (address.getAddressDetail() instanceof String) && (address.getProvince() instanceof String)
						&& (address.getCity() instanceof String) && (address.getArea() instanceof String)) {
					result.element("code", 200);
					result.element("message", "success");
					// return result;
				} else {

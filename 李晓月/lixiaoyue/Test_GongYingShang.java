...
//添加供应商	
	...
	public void Commodity(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("商品管理"));
	}	
	//点击查看商品列表
	@Test(priority = 3)
	public void List2(){
	...
	public void Order(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("商品订单"));
	}	
	//查看退款单
	@Test(priority = 5)
	public void Refund(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("已付款并取消单列表"));
	}
	//点击查看退换货
	@Test(priority = 6)
	public void Goods(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("退货退款管理"));
	}	
		//查看发票列表
	@Test(priority = 7)
	public void List(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("商业用途必须购买正版,使用盗版将追究法律责任"));
	}
	//点击体现申请
	@Test(priority = 8)
	public void Apply(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("提现申请管理"));
	}
	//点击查看转款列表
	@Test(priority = 9)
	public void Transfer(){
		...
		Assert.assertTrue(getDriver().getPageSource().contains("供应商汇款记录"));
	}
	//点击查看结算记录
	@Test(priority = 10)
	public void Settlement() throws Exception{
		...
		Assert.assertTrue(getDriver().getPageSource().contains("供应商结算管理"));
	}	
}
